...
export default function WomensCollectionPage() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const highestPrice = Math.max(...womensProducts.map(p => p.price || 0))
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== "all") filterCount++
                        ...
                        {colors.map((color) => (
                          <div
                            ...
                            className={`w-8 h-8 rounded-full cursor-pointer flex items-center justify-center
                              ...
                              ${selectedColors.includes(color.id) ? 'ring-2 ring-offset-2 ring-amber-900' : ''}
                            `}
                          ...
                          >
                            {selectedColors.includes(color.id) && (
                              <CheckIcon size={14} className={`${['white', 'yellow', 'gold'].includes(color.id) ? 'text-black' : 'text-white'}`} />
                            )}
                          </div>
                        ))}
                      </div>
